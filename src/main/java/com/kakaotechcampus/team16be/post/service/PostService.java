...
public class PostService {
    ...
    public PostResponse createPost(Long groupId, Long memberId, PostRequest request) {
        ...
        Post post = new Post(request.title(), request.content(), request.images(), member, group);
    ...
    public PostLikeResponse likePost(Long postId, Long memberId) {
        Member member = memberRepository.findById(memberId).orElseThrow(() -> new RuntimeException("Member not found"));
        Post post = postRepository.findById(postId).orElseThrow(() -> new RuntimeException("Post not found"));
        postLikeRepository.findByPostAndMember(post, member).ifPresent(like -> {
    ...
    public PostLikeResponse unlikePost(Long postId, Long memberId) {
        Member member = memberRepository.findById(memberId).orElseThrow(() -> new RuntimeException("Member not found"));
        Post post = postRepository.findById(postId).orElseThrow(() -> new RuntimeException("Post not found"));
        PostLike postLike = postLikeRepository.findByPostAndMember(post, member).orElseThrow(() -> new RuntimeException("Not liked yet"));
